...
export const getArticlesAnalysis = async (req: Request, res: Response) => {
  try {
    ...
    const cleanedArticles = rawArticles.filter((art: any) => {
      let isValid = true;
      if (!art.judul) {
        nullJudul++;
        isValid = false;
    ...
    const categoryDistribution: Record<string, { count: number; percentage: number }> = {};
    ...
    cleanedArticles.forEach((art: any) => {
      const cat = art.kategori.trim();
      let standardCat = "Other";
      if (/adhd/i.test(cat)) standardCat = "ADHD";
      else if (/autis/i.test(cat)) standardCat = "Autisme";
      else if (/speech/i.test(cat) || /bicara/i.test(cat)) standardCat = "Speech Delay";
    ...
    Object.keys(categoryDistribution).forEach(key => {
      if (cleanCount > 0) {
        categoryDistribution[key].percentage = Number(
          ((categoryDistribution[key].count / cleanCount) * 100).toFixed(2)
        );
      }
    });
    ...
    const timelineTrend: Record<string, { [category: string]: number }> = {};
    ...
    cleanedArticles.forEach((art: any) => {
      ...
      if (standardCat === "Other" || !keyTermsMap[standardCat]) return;
    ...
    const formattedTermFrequencies = Object.entries(termFrequencies).reduce((acc, [cat, terms]) => {
      ...
      return acc;
    }, {} as Record<string, Array<{ text: string; value: number }>>);
